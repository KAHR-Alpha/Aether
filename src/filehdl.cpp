...
bool is_directory(std::string const &dir)
{
    ...
    DWORD f_attr=GetFileAttributesA(dir.c_str());
    
         if(f_attr==INVALID_FILE_ATTRIBUTES) return false;
    else if(f_attr & FILE_ATTRIBUTE_DIRECTORY) return true;
...
[[deprecated]]
bool make_directory(std::string const &dir)
{
    ...
    for(size_t i=0;i<dir.size();i++)
        if(dir[i]=='/') sep.push_back(i);
    
    if(dir[dir.size()-1]!='/')
        sep.push_back(dir.size());
    ...
    {
        std::string tmp_dir=dir.substr(0,sep[i]);
        
        #ifdef UNIX_PLATFORM
        result=mkdir(tmp_dir.c_str(),0777);
        #endif
        #ifdef _WIN32
        result=CreateDirectoryA(tmp_dir.c_str(),NULL);
        #endif
    }
...
[[deprecated]]
void proper_dirname_convertion(std::string &dir)
{
    int i,N=dir.size();
...
[[deprecated]]
void proper_filename_convertion(std::string &dir)
{
    int i,N=dir.size();
